...
class VIVIP(Approximator):
    ...
    def init(self,potential_fn, marginalized, remained, translate, num_sample, *args, rng_key = 0, step_size = 1e-4, steps = 100000, theta_flows = 2, x_flows = 1, hidden_scale = 2, hidden_dim_scale=8, **kwargs):
        ...
        guide_trace = trace(guide_init).get_trace(jnp.zeros(self.in_dim),jnp.zeros((self.z_dim,num_sample)))
        params = {}
        inv_transforms = {}
        ...
        #)
        #print(scale_tril)
        init_state = SVIState(optimizer.init((params,init_loc,init_scale, init_lambda)), None, state_seed)
        ...
        def update(svi_state, optimizer, *args, **kwargs):
            ...
            def loss_fn(params):
                ...
                def single_sample_logpdf(z2):
                    ...
                    return -self.potential_fn(*args, **the_kwargs)(dict)
        ...
        self.state = state
        self.params, loc, scale, self.lamb = optimizer.get_params(state.optim_state)
        print(loc,scale)
        print(self.lamb)
