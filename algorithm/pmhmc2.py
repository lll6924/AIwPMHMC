...
class PMHMC2:
    ...
    def compile(self,
        ...
        if self.approximate_strategy == 'DeepVariationalInference':
            self.approximator.init(self.potential_fn, marginalized, remained, full_translate, self.num_sample, *self.model_args, rng_key=approximator_key, step_size=self.vi_step_size, steps=self.vi_steps, **self.model_kwargs)
        elif self.approximate_strategy == 'VariationalInference':
            self.approximator.init(self.potential_fn, marginalized, remained, full_translate, self.num_sample, *self.model_args, rng_key=approximator_key, step_size=self.vi_step_size, steps=self.vi_steps, **self.model_kwargs)
        elif self.approximate_strategy == 'VIVIP':
            lamb = self.approximator.init(self.potential_fn, marginalized, remained, full_translate, self.num_sample, *self.model_args, rng_key=approximator_key, step_size=self.vi_step_size, steps=self.vi_steps, **self.model_kwargs)
            self.model_kwargs['lamb'] = lamb
        elif self.approximate_strategy =='VariationalInferenceHierarchical':
        ...
        def neg_log_prob(z):
            ...
            for i in range(len(remain_id)):
                _i = remain_id[i]
                unchanged_dict[self.params[_i]] = jnp.reshape(z1[remain_l[i]:remain_r[i]], self.par_shapes[_i])
            def single_sample_logpdf(marginals):
                dict = unchanged_dict
                for i in range(len(marginalize_id)):
                    _i = marginalize_id[i]
                    dict[self.params[_i]] = jnp.reshape(marginals[marginalize_l[i]:marginalize_r[i]],
                                                        self.par_shapes[_i])
            ...
            log_mean_sqr = log_mean_exp(x*2)
            #bigger = jnp.max(jnp.array([log_mean*2,log_mean_sqr]))
            return - log_mean  - cond_logdet - jnp.sum(dist.Normal(0,1).log_prob(z2))
        ...
        def postprocess(sites, zs, key):
            ...
            def get_sample(theta,z,rng_key):
                ...
                for i in range(len(remain_id)):
                    _i = remain_id[i]
                    unchanged_dict[self.params[_i]] = jnp.reshape(theta[remain_l[i]:remain_r[i]], self.par_shapes[_i])
                ...
                logpdfs = jnp.asarray(logpdfs)
                x = logpdfs - qlogpdf
                id = random.categorical(rng_key, x)
                return samples[id]
